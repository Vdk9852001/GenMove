...
class FieldResult:
    ...
    matched_details:   list  = field(default_factory=list)
...
class MaterialValidator:
    ...
    def _validate_field_fast(
        ...
        # Build complete row-level details. ``None`` means no report limit.
        mismatches = []

        def take(frame, remaining=None):
            if max_rows is None:
                return frame
            limit = max_rows if remaining is None else max(0, remaining)
            return frame.head(limit)

        if miss_src_count > 0:
            for r in take(m[miss_src_m]).to_dict("records"):
                mismatches.append({
        ...
        if miss_tgt_count > 0 and (max_rows is None or len(mismatches) < max_rows):
            remaining = None if max_rows is None else max_rows - len(mismatches)
            for r in take(m[miss_tgt_m], remaining).to_dict("records"):
                mismatches.append({
                    "material":     _ck_label(r[_CK]),
        ...
        if mismatched > 0 and (max_rows is None or len(mismatches) < max_rows):
            remaining = None if max_rows is None else max_rows - len(mismatches)
            mdf = take(m[valid][mismatch_mask], remaining)
            if tolerance is not None:
        ...
        # Exact/within-tolerance matches for PASS-sheet and UI drill-down.
        match_full = pd.Series(False, index=m.index)
        match_full.loc[both_null[both_null].index] = True
        match_full.loc[match_mask[match_mask].index] = True
        matched_rows = []
        for record in take(m[match_full]).to_dict("records"):
            source_value = record.get(sv_col, "")
            target_value = record.get(tv_col, "")
            matched_rows.append({
                "material": _ck_label(record[_CK]),
                "source_value": "(blank)" if pd.isna(source_value) else str(source_value),
                "target_value": "(blank)" if pd.isna(target_value) else str(target_value),
                "result": "Within tolerance" if tolerance is not None else "Exact match",
            })
        ...
        return FieldResult(
            ...
            is_numeric=(tolerance is not None), is_key_field=is_key_field,
            tolerance_used=tolerance, pass_threshold=self.pass_threshold,
            mismatch_details=mismatches, matched_details=matched_rows,
        )
